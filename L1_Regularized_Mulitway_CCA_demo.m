...
clear
close all;
%% L1-Regularized Multiway CCA (L1-MCCA) in SSVEP frequency detection (demo code)
...
for sbj=1:35
    ...
    lambda=0.03;    % reqularization term 
    ...
    for run=1:n_run
        ...
        for j=1:numel(fstim)
            [w1(:,j),w3(:,j),v1(:,j)]=...
                mySMCCA(Xref(:,:,j),SSVEPdata_sep(:,:,idx_traindata,j),max_iter,iniw3,n_comp,lambda);
